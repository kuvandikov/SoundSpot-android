...
import com.kuvandikov.base.util.extensions.plus
...
import com.kuvandikov.soundspot.downloader.artworkFromFile
import com.kuvandikov.soundspot.playback.AudioFocusHelperImpl
import com.kuvandikov.soundspot.playback.AudioQueueManagerImpl
...
import com.kuvandikov.soundspot.playback.createDefaultPlaybackState
...
import com.kuvandikov.soundspot.playback.models.QueueState
import com.kuvandikov.soundspot.playback.models.toMediaId
import com.kuvandikov.soundspot.playback.models.toMediaMetadata
import com.kuvandikov.soundspot.playback.position
import com.kuvandikov.soundspot.playback.repeatMode
import com.kuvandikov.soundspot.playback.shuffleMode
...
typealias OnMetaDataChanged = SoundspotPlayer.() -> Unit
...
interface SoundspotPlayer {
    ...
    fun release()
    fun onPlayingState(playing: OnIsPlaying<SoundspotPlayer>)
    fun onPrepared(prepared: OnPrepared<SoundspotPlayer>)
    fun onError(error: OnError<SoundspotPlayer>)
    fun onCompletion(completion: OnCompletion<SoundspotPlayer>)
...
@Singleton
class SoundspotPlayerImpl @Inject constructor(
...
) : SoundspotPlayer, CoroutineScope by MainScope() {
    ...
    private var isPlayingCallback: OnIsPlaying<SoundspotPlayer> = { _, _ -> }
    private var preparedCallback: OnPrepared<SoundspotPlayer> = {}
    private var errorCallback: OnError<SoundspotPlayer> = {}
    private var completionCallback: OnCompletion<SoundspotPlayer> = {}
    ...
    private val mediaSession = MediaSessionCompat(context, context.getString(R.string.app_name), null, pendingIntent).apply {
        setCallback(
            MediaSessionCallback(this, this@SoundspotPlayerImpl, audioFocusHelper)
        )
    ...
    init {
        ...
        audioPlayer.onPrepared {
            preparedCallback(this@SoundspotPlayerImpl)
            launch {
        ...
        audioPlayer.onCompletion {
            completionCallback(this@SoundspotPlayerImpl)
        ...
        audioPlayer.onError { throwable ->
            ...
            errorCallback(this@SoundspotPlayerImpl, throwable)
            isInitialized = false
            updatePlaybackState {
    ...
    override fun onPlayingState(playing: OnIsPlaying<SoundspotPlayer>) {
        this.isPlayingCallback = playing
    }

    override fun onPrepared(prepared: OnPrepared<SoundspotPlayer>) {
        this.preparedCallback = prepared
    }

    override fun onError(error: OnError<SoundspotPlayer>) {
        this.errorCallback = error
    }

    override fun onCompletion(completion: OnCompletion<SoundspotPlayer>) {
